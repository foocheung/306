...
process_dataset <- function(config_file) {
  ...
  for (i in 1:length(lanes)) {
    demuxbestList[[i]] <- read.table(paste("DEMUXALOT2", "/multi_config_", lanes[[i]], "/assignments_refined.tsv", sep = ""), sep = "\t", header = TRU
E, col.names = c("sBARCODE", "sCALL"))
  }
  ...
  B1_US_SeuratObj <- AddMetaData(object = B1_US_SeuratObj, metadata = Matrix::colSums(B1_US_SeuratObj[mito.genes,])/Matrix::colSums(B1_US_SeuratObj), 
...
ggsave(paste(config$output_prefix, "_pre-filtering.pdf", sep = ""), height = 10, plot = VlnPlot(B1_US_SeuratObj, c("percent.mito", "percent_ribo", "pe
rcent_hb", "nFeature_RNA", "nCount_RNA"), pt.size = 0, ncol = 1 ,group.by="Lane"))
  ...
  DSB.ADT.list <- DSBNormalizeProtein(cell_protein_matrix = as.matrix(B1_US_SeuratObj_pos[["CITE"]]@counts), empty_drop_matrix = as.matrix(B1_US_Seura
tObj_neg[["CITE"]]@counts), denoise.counts = TRUE, isotype.control.name.vec = isotype.control.name.vec)
  
  B1_US_SeuratObj_pos <- SetAssayData(B1_US_SeuratObj_pos, assay = "CITE", new.data = DSB.ADT.list, slot = "data")
 

ggsave(paste(config$output_prefix, "_pos_post-filtering.pdf", sep = ""), height = 10, plot = VlnPlot(B1_US_SeuratObj_pos, c("percent.mito", "percent_r
ibo", "percent_hb", "nFeature_RNA", "nCount_RNA"), pt.size = 0, ncol = 1 ,group.by="Lane"))
...
ggsave(paste(config$output_prefix,"_UMAP.pdf" , sep=""),height =3, width=length(B1_US_SeuratObj_pos$Lane  %>% unique()) + 1 * 3, plot=scCustomize::Dim
Plot_scCustom(seur, split.by = "Lane",label = FALSE,num_columns=length(B1_US_SeuratObj_pos$Lane  %>% unique()) + 1, reduction = "rna.umap", group.by =
 "monaco_ann1", split_seurat = TRUE ))
