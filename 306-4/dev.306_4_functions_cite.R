...
read_demuxalot_files <- function(lanes, prefix_dir,demuxdir2, B1_US_merge) {
  ...
  for(i in 1:length(lanes)){
    demuxbestList[[i]] = read.table(paste(demuxdir2,"/multi_config_",
                                          lanes[[i]],"/assignments_refined.tsv", sep = ""), sep = "\t", header = TRUE, col.names = c("sBARCODE", "sCALL"))
    
    }
  names(demuxbestList) <- lanes
  
  for(i in 1:length(lanes)){
    demuxbestList[[i]]$sNewBarcode = paste(substr(demuxbestList[[i]]$sBARCODE, 
                                                 start = 1, stop = 17),
                                          lanes[[i]],"_306_4", sep = "")
  demuxbestList[[i]] <- demuxbestList[[i]] %>%
  mutate(sDROPLET.TYPE = ifelse(grepl("\\+", sCALL), "DBL",
                                  ifelse(grepl("AMB", sCALL), "AMB", "SNG")))
 # mutate(sDROPLET.TYPE = ifelse(grepl("\\+", sCALL), "DBL", "SNG"))
   # demuxbestList[[i]]$DROPLET.TYPE <- "SNG"  # Assign 'SNG' to all rows by default
  #  demuxbestList[[i]]$DROPLET.TYPE[grepl("\\+", demuxbestList[[i]]$sCALL)] <- "DBL"  # Assign 'DBL' where '+' is present in 'sCALL'
  }
  
  
  #check the overlay of cells between seuratobj and demux list
  for(i in 1:length(SeuratObj)){
    length(which(colnames(SeuratObj[[i]]) %in% demuxbestList[[i]]$sNewBarcode))
    print(setdiff(colnames(SeuratObj[[i]]), demuxbestList[[i]]$sNewBarcode))
  }
...
read_demuxalot_files2 <- function(lanes, prefix_dir,demuxdir2, B1_US_merge) {
  SeuratObj<-B1_US_merge
...
#data <- read.table("your_file_path.csv", header = TRUE, col.names = paste0(names(read.table("your_file_path.csv", nrows = 1)), "s"))

    demuxbestList[[i]] = read.table(paste(demuxdir2,
					"/multi_config_",
                                        lanes[[i]],
					"/posterior_probabilities_refined.tsv", sep = ""), sep = "\t", header = TRUE #, col.names = 
				  # paste0(names(read.table(
			#		paste(demuxdir2,
                        #                "/multi_config_",
                        #                lanes[[i]],
                        #                "/posterior_probabilities_refined.tsv", sep = "", sep = "\t", header = TRUE)), "s")
			#		)
                                     )
  ...
  for(i in 1:length(lanes)){
    demuxbestList[[i]]$s2NewBarcode = paste(substr(demuxbestList[[i]]$BARCODE,
                                                 start = 1, stop = 17),
                                          lanes[[i]],"_306_4", sep = "")
  #  demuxbestList[[i]] <- demuxbestList[[i]] %>%
  #    mutate(sDROPLET.TYPE = ifelse(grepl("\\+", sCALL), "DBL", "SNG"))
  ...
  #check the overlay of cells between seuratobj and demux list
  for(i in 1:length(SeuratObj)){
    length(which(colnames(SeuratObj[[i]]) %in% demuxbestList[[i]]$s2NewBarcode))
    print(setdiff(colnames(SeuratObj[[i]]), demuxbestList[[i]]$s2NewBarcode))
  }

  demuxbestdf <- plyr::ldply(demuxbestList, data.frame)
  rownames(demuxbestdf) <- demuxbestdf$s2NewBarcode
...
#' Read raw h5 Files
read_raw_h5_files <- function(lanes, rawdir,prefix_dir) {
  B1_US_data <- list()
  for(i in 1:length(lanes)){
    B1_US_data[[i]] = Read10X_h5(paste(rawdir, prefix_dir, lanes[i], "_raw_feature_bc_matrix.h5", sep = ""))
  }
  return(B1_US_data)
}
...
DSB_Normalize <- function(ObjList) {
  ...
  ObjList_SNG = list()
  for(i in 1:length(ObjList)){
    ObjList_neg[[i]] <- subset(ObjList[[i]], DROPLET.TYPE == "AMB" &  sCALL == "DBL" )
    # ObjList_neg[[i]] <- subset(ObjList[[i]], cells = sample(Cells(ObjList_neg[[i]]),1000))
    ObjList_SNG[[i]] <- subset(ObjList[[i]], DROPLET.TYPE == "SNG")
  }
